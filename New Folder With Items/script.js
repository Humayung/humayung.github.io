...
const interpolateColors = () => {
	...
	for (var i = 0; i < colors.length; i++) {
		var color = toString(
			lerpColor(rgb(oldColors[i]), rgb(colors[i]), colorPos)
		);
...
c.addEventListener(
	"click",
	function () {
		window.open("fractal.html", "_self");
	},
	false
);
...
const draw = () => {
	...
	var text =
		"ostrich scan expose grab damp diet angle circle collect visit wheat brief";
